...
jQuery(document).ready(function($){
    ...
    window.powered_by = '';
    window.powered_by_url = '';
    window.get_plugin_url = '';
    //get plugin URL NAME /get plugin URL
    $.get(ajax_url,{
        action : 'fetch_plugin_data'
    }, function(data){
        window.powered_by = data.powered_by;
        window.powered_by_url = data.powered_by_url;
        window.get_plugin_url = data.get_plugin_url;
        $('#powered-by a').attr('href',window.powered_by_url).text(window.powered_by);
        $('#get-the-plugin a').attr('href',window.get_plugin_url);
    },'json');
    ...
    $('a.tab').click(function(e){
        ...
        e.preventDefault();
        var inner = $('#fixedheight').html();
        $('#fixedheight').html('').html('<p class="center">Loading...</p>');
        $(this).siblings().css({
            'font-weight':'normal',
            color : '#ccc',
            background : '#fff'
        ...
        $('#bitcoin-data').append('<div class="loader">Loading...</div>');
        $('#bitcoin-widget .loader').height($('#bitcoin-widget').height() - 40);
        var name = $(this).attr('data-name');
        jQuery("#chart").empty().html('Loading chart....');
        show_chart(name);
        ...
        $.get(ajax_url,{
            ...
            //clear loader
            $('#fixedheight').html(inner);
            jQuery("#chart").html('loading chart...');
            $('#bitcoin-widget .loader').remove();
            $('#last-price').html(' <h2>$'+parseFloat(data.ticker.buy).toFixed(3)+'</h2>');
            $('#bitcoin-data').html('<ul>\
                                                    <li>Buy : $'+ parseFloat(data.ticker.buy).toFixed(4)+'</li>\
                                                    <li>Sell : $'+parseFloat(data.ticker.sell).toFixed(4)+'</li>\
                                                    <li>High : $'+parseFloat(data.ticker.high).toFixed(4)+'</li>\
                                                    <li>Low : $'+parseFloat(data.ticker.low).toFixed(4)+'</li>\
                                                    <li>Volume : '+parseInt(data.ticker.vol)+'</li>\
                                                </ul>');
            $('#link-row').html('<span class="last_updated">Last updated: <span class="timeago" data-livestamp="'+ data.ticker.updated +'"></span></span>');
            $('#powered-by span').text(window.powered_by);
            $('#get-the-plugin a').attr('href',window.get_plugin_url);
...
function show_chart(name){
    ...
    function(response){
        jQuery("#chart").empty();
        jQuery.plot("#chart", [response]);
        jQuery('div.flot-x-axis').remove();
    },'json');
}
